    ...
    >
            ...
            <div class="flex flex-wrap space-x-1" x-cloak x-show="selected.length > 0">
                <template x-for="(key, index) in selected" :key="index">
                    <div class="text-gray-800 rounded-full truncate bg-blue-300 px-2 py-0.5 my-0.5 flex flex-row items-center">
                        ...
                        <div x-show="!disabled" class="w-4 cursor-pointer" @click.prevent.stop="deselectOption(index)">
        ...
        @else
            <div class="flex flex-wrap" x-cloak x-show="selected">
                <div class="text-gray-800 rounded-full truncate bg-blue-300 px-2 py-0.5 my-0.5 flex flex-row items-center">
                    ...
                    <div x-show="!disabled" class="h-4 cursor-pointer" @click.prevent.stop="deselectOption()">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="h-4 fill-current">
                            <path d="M12 2c5.514 0 10 4.486 10 10s-4.486 10-10 10-10-4.486-10-10 4.486-10 10-10zm0-2c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm6 16.538l-4.592-4.548 4.546-4.587-1.416-1.403-4.545 4.589-4.588-4.543-1.405 1.405 4.593 4.552-4.547 4.592 1.405 1.405 4.555-4.596 4.591 4.55 1.403-1.416z"/>
                        </svg>
                    </div>
        ...
        >
            ...
            <div x-ref="dropdown" class="relative z-30 p-2 overflow-y-auto max-h-60">
                <div x-cloak x-show="Object.keys(options).length === 0" x-text="emptyOptionsMessage"></div>
        ...
        function AlpineSelect(config) {
            return {
                ...
                increaseIndex: function() {
                    this.currentIndex = (this.currentIndex + 1) % Object.keys(this.options).length;
                },

                decreaseIndex: function() {
                    this.currentIndex = (this.currentIndex - 1 + Object.keys(this.options).length) % Object.keys(this.options).length;
                },
            }
        }
    </script>
