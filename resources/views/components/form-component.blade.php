        ...
        @foreach ($fields as $field)
            <div class="w-full md:w-{{ $field['width'] ?? 6 }}/12 px-2 mb-6">
                ...
                @elseif ($field['type'] === 'invisible')
                    <input type="text"  id="{{ $field['name'] }}" name="{{ $field['name'] }}" value="{{ old($field['name'], $values->{$field['name']} ?? '') }}" class="bg-gray-50 border invisible border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="{{ $field['placeholder'] ?? '' }}" {{ $field['required'] ? 'required' : '' }} />
                @elseif ($field['type'] === 'checkbox')
                    <div class="flex items-start mb-6">
                        ...
                        <label for="{{ $field['name'] }}" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ $field['label'] }}</label>
                    </div>
                    ...
                    <select id="{{ $field['name'] }}" 
                        ...
                        @foreach ($field['options'] as $value => $label)
                            <option value="{{ $value }}" 
                                ...
                                @else
                                    {{ old($field['name'], $values->{$field['name']} ?? '') == $value ? 'selected' : '' }}
                                @endif
                                >
                                {{ $label }}
                            </option>
