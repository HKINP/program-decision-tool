        ...
        <div class="border border-gray-200 rounded-lg p-4 mb-4">
            ...
            <div class="mb-4">
                ...
                <textarea id="key_barriers" name="key_barriers[]" rows="4"
                    ...
                    placeholder="Write your notes here...">{{ old('key_barriers.'.$index) }}</textarea>
            </div>

            <div id="activities-container-{{ $question->question_id }}" class="space-y-4">
                @foreach(old('proposed_activities.'.$index, ['']) as $activityIndex => $activity)
                <div class="border border-gray-300 rounded-lg p-4 bg-gray-50">
                    <div class="grid grid-cols-1 gap-4 mb-2">
                        <div class="flex flex-col w-full">
                            <label for="activity-text-{{ $question->question_id }}-{{ $activityIndex }}" class="text-sm font-medium text-gray-700">Activity Details</label>
                            <textarea id="activity-text-{{ $question->question_id }}-{{ $activityIndex }}" name="proposed_activities[{{ $index }}][]" rows="3"
                                ...
                                placeholder="Write your activity details here...">{{ $activity }}</textarea>
                        </div>
                        <input type="number" name="indicator_id[]" value="{{ $question->question_id }}" hidden>
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                            <div class="flex flex-col w-full">
                                <label for="targetted-for-option-{{ $question->question_id }}-{{ $activityIndex }}" class="text-sm font-medium text-gray-700">Targetted For</label>
                                <select id="targetted-for-option-{{ $question->question_id }}-{{ $activityIndex }}" name="targeted_for[{{ $index }}][]"
                                    class="bg-white border border-gray-300 rounded-lg p-2 text-sm w-full">
                                    <option value="all" {{ old('targeted_for.'.$index.'.'.$activityIndex) == 'all' ? 'selected' : '' }}>All</option>
                                    <option value="vulnerable" {{ old('targeted_for.'.$index.'.'.$activityIndex) == 'vulnerable' ? 'selected' : '' }}>Vulnerable</option>
                                </select>
                            </div>
                            <div class="flex flex-col w-full">
                                <label for="platform-option-{{ $question->question_id }}-{{ $activityIndex }}" class="text-sm font-medium text-gray-700">Platform</label>
                                <select id="platform-option-{{ $question->question_id }}-{{ $activityIndex }}" required multiple name="platforms_id[{{ $index }}][{{ $activityIndex }}][]"
                                    class="bg-white border border-gray-300 multipleselect rounded-lg p-2 text-sm w-full">
                                    @foreach($platforms as $platform)
                                    <option value="{{ $platform->id }}" {{ collect(old('platforms_id.'.$index.'.'.$activityIndex))->contains($platform->id) ? 'selected' : '' }}>
                                        {{ $platform->platforms }}
                                    </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="flex flex-col w-full">
                                <label for="notes-text-{{ $question->question_id }}-{{ $activityIndex }}" class="text-sm font-medium text-gray-700">Remarks</label>
                                <textarea id="notes-text-{{ $question->question_id }}-{{ $activityIndex }}" name="remarks[{{ $index }}][]" rows="1"
                                    class="block w-full text-sm text-gray-900 bg-white rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                                    placeholder="Write your notes here...">{{ old('remarks.'.$index.'.'.$activityIndex) }}</textarea>
                            </div>
                        </div>
...
<script>
    function addActivity(questionId) {
        ...
        const activityCount = container.childElementCount;
        const activityHtml = `
            <div class="border border-gray-300 rounded-lg p-4 bg-gray-50">
                <div class="grid grid-cols-1 gap-4 mb-2">
                    <div class="flex flex-col w-full">
                        <label for="activity-text-${questionId}-${activityCount}" class="text-sm font-medium text-gray-700">Activity Details</label>
                        <textarea id="activity-text-${questionId}-${activityCount}" name="proposed_activities[${questionId}][]" rows="3"
                            class="block w-full text-sm text-gray-900 bg-white rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        <div class="flex flex-col w-full">
                            ...
                            <select id="targetted-for-option-${questionId}-${activityCount}" name="targeted_for[${questionId}][]"
                                class="bg-white border border-gray-300 rounded-lg p-2 text-sm w-full">
                        ...
                        <div class="flex flex-col w-full">
                            ...
                            <textarea id="notes-text-${questionId}-${activityCount}" name="remarks[${questionId}][]" rows="1"
                                class="block w-full text-sm text-gray-900 bg-white rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                    ...
                    <div class="flex justify-end items-center">
                        <button type="button" class="text-red-500 hover:text-red-700" onclick="removeActivity(this)">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7H5M9 11v6m6-6v6M5 7h14M4 7h16M4 7l1 13h14L20 7M10 11v6m4-6v6m-8-6v6"></path>
                            </svg>
                        </button>
                    </div>
                </div>
    ...
    function removeActivity(button) {
        const activity = button.closest('.border');
        activity.remove();
    }
