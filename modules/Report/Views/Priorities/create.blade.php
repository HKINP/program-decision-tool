<x-app-layout>
    ...
    <tbody class="rounded-lg">
        ...
        <tr>
            ...
            <span class="bg-blue-100 text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full dark:bg-blue-900 dark:text-blue-300">{{ $question->targetGroup->target_group }}</span>
</td>
            <td class="border text-sm border-gray-200 p-2">
            <span class="bg-gray-100 text-gray-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full dark:bg-gray-700 dark:text-gray-300">{{ $question->thematicArea->thematic_area }}</span>
            ...
            <input type="number" name="response_all_{{ $question->id }}" class="border-none text-sm w-20 rounded-lg p-1 status-input" placeholder="N/A" />
        </td>
            <td class="border text-sm border-gray-200 p-2">
                <input type="number" name="response_underserved_{{ $question->id }}" class="border-none text-sm w-20  rounded-lg p-1" placeholder="N/A" />
            </td>
            <td class="border text-sm border-gray-200 p-2">
                <select name="priority_{{ $question->id }}" class="text-sm w-full border-none rounded-lg p-1">
...
$(document).ready(function() {
    // Function to update the cell styles based on the input value
    function updateCellStyle() {
        ...
        $('.status-input').each(function() {
            ...
            var $td = $(this).closest('td'); // Find the closest <td> element

            // Reset the styles first
            $td.removeClass('bg-red-500 text-white bg-yellow-500 bg-green-500 text-black').addClass('bg-white text-black');

            // Apply styles based on the input value
            if (inputValue > 50) {
                $td.addClass('bg-red-500 text-white');
            } else if (inputValue <= 50 && inputValue >= 30) {
                $td.addClass('bg-yellow-500 text-black');
            } else if (inputValue < 30 && inputValue !== '') {
                $td.addClass('bg-green-500 text-white');
            }
        });
    }
    ...
    // Run the function whenever the input value changes
